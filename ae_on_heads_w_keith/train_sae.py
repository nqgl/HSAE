...
def train(encoder :z_sae.AutoEncoder, cfg :z_sae.AutoEncoderConfig, buffer :z_sae.Buffer, model :HookedTransformer):
    ...
    try:
        ...
        for i in tqdm.trange(num_batches):
            ...
            loss = encoder.get_weird_loss(n = n, down_flex = flex )
            ...
            loss.backward()
            n = max(MIN_N, (n + l0_norm.item() - 1) / 2)
...
ae_cfg = z_sae.AutoEncoderConfig(site="z", act_size=512, layer=1,
                            l1_coeff=16e-4, dict_mult=32, batch_size=256, beta2=0.99, seed = 55,
                            nonlinearity=("relu", {}), flatten_heads=True, buffer_mult=10000, buffer_refresh_ratio=0.25,
                            lr=3e-5, cosine_l1={"period": 78300, "range" : 0.0125}) #original 3e-4 8e-4 or same but 1e-3 on l1
