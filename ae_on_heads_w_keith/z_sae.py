...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, cache_l0 = True, cache_acts = False, record_activation_frequency = False):
        ...
        self.l1_loss_cached = torch.pow(acts.float().abs() + 1e-5, 0.8) # TODO fix this embarrasment
        self.l2_loss_cached = (x_reconstruct.float() - x.float()).abs().mean(-1) # don't tell anyone I tried this
        self.l2_loss_cached = ((x_reconstruct.float() - x.float()).pow(2).mean(-1) + 1e-5).pow(0.4)
        if cache_l0:
    ...
    def get_loss(self, n=1, down_flex=1):
        ...
        l0l1_multiplier = torch.max(torch.tensor(0, device="cuda"), l0l1_multiplier - n) / 5 + 1
