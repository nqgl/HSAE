...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, cache_l0 = True, cache_acts = False, record_activation_frequency = True):
        ...
        if cache_l0:
            self.l0_norm_cached = (acts > 0).float().sum() / acts.shape[0]
        else:
            self.l0_norm_cached = None
        if cache_acts:
        ...
        if record_activation_frequency:
            print(acts.shape)
            self.activation_frequency += (acts > 0).float().mean(dim=(1,0))
    ...
    def get_loss(self):
        return torch.sum(self.l2_loss_cached + self.l1_coeff.reshape(1,-1) * self.l1_loss_cached)
