...
class HierarchicalAutoEncoderConfig(AutoEncoderConfig):
    ...
    features_per_sae_per_layer: List[Optional[int]] = field(
        default_factory=lambda: [None, 32]
    )
    num_layers: int = 2
    sublayer_cfgs: List["HierarchicalAutoEncoderLayerConfig"] = field(
    ...
    def __post_init__(self):
        ...
        if len(self.layer_cfg_params_per_layer) < self.num_layers:
            self.layer_cfg_params_per_layer += [
                {}
                for _ in range(self.num_layers - len(self.layer_cfg_params_per_layer))
            ]
        ...
        if self.sublayer_cfgs == []:
            ...
            for i in range(0, self.num_layers - 1):
                ...
                hlcfg = HierarchicalAutoEncoderLayerConfig(**cfg_params)
                num_features_at_layer *= hlcfg.d_dict
                self.sublayer_cfgs.append(hlcfg)
        elif isinstance(self.sublayer_cfgs[0], dict):
            self.sublayer_cfgs = [
                HierarchicalAutoEncoderLayerConfig(**cfg) for cfg in self.sublayer_cfgs
            ]
