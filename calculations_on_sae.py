...
def get_recons_loss(model, encoder, buffer, num_batches=5, local_encoder=None):
    ...
    for i in range(num_batches):
        ...
        recons_loss = model.run_with_hooks(
            ...
            return_type="loss",
            fwd_hooks=[
                (encoder.cfg.act_name, partial(replacement_hook, encoder=local_encoder))
            ],
        )
        ...
        zero_abl_loss = model.run_with_hooks(
            tokens,
            return_type="loss",
            fwd_hooks=[(encoder.cfg.act_name, zero_ablate_hook)],
        )
...
def get_freqs(model, encoder, buffer, num_batches=25, local_encoder=None):
    if local_encoder is None:
        local_encoder = encoder
    act_freq_scores = torch.zeros(local_encoder.d_dict, dtype=torch.float32).to(
        encoder.cfg.device
    )
    total = 0
